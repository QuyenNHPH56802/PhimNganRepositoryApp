"""Anthropic Claude-compatible HTTP translation provider."""
...
class ClaudeCompatibleHttpProvider(Provider[TranslationInput, TranslationResponse]):
    id = "claude_compatible_http"
    ...
    def fingerprint(self, payload: TranslationInput) -> ArtifactSignature:
        ...
        return ArtifactSignature(
            ...
            model_id=cfg.model_id if cfg else "claude_compatible_http",
    ...
    async def run(self, payload: TranslationInput, *, ctx: ProviderContext) -> TranslationResponse:
        cfg = payload.config
        if cfg is None:
            raise CapabilityUnsupported("translate-missing-config", "Claude provider requires TranslationProviderConfig")
        ...
        headers = {"x-api-key": api_key, "anthropic-version": CLAUDE_API_VERSION, "content-type": "application/json"}
        async with httpx.AsyncClient(timeout=cfg.timeout_s) as client:
            response = await client.post(f"{cfg.base_url.rstrip('/')}/v1/messages", json=body, headers=headers)
        if response.status_code >= 400:
            raise RuntimeError(f"claude-http-{response.status_code}: {response.text[:256]}")
        data = response.json()
