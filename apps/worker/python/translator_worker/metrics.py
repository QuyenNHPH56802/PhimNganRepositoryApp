...
try:
    ...
    TTS_GENERATE_SECONDS = Histogram(
        ...
        "Wall-clock time spent inside the TTS provider (excluding cache hits)",
        labelnames=("provider",),
    ...
    TTS_AUDIO_SECONDS = Histogram(
        "tts_audio_seconds",
        "Total audio duration generated by the TTS provider (post-cache)",
        labelnames=("provider",),
    ...
    CACHE_HIT = _stub()
    CACHE_MISS = _stub()
    TTS_GENERATE_SECONDS = _stub()
    TTS_AUDIO_SECONDS = _stub()
...
def observe_tts(*, provider: str, generate_seconds: float, audio_seconds: float | None = None) -> None:
    """Record TTS generation wall-clock time and (optionally) audio length.

    ``audio_seconds`` may be ``None`` when the worker cannot easily compute
    the synthesized length (e.g. raw bytes from an opaque SDK). Callers
    should estimate it from the provider's response when possible.
    """
