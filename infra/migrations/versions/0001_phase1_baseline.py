...
Implements the ERD defined in docs/ERD.md (only metadata tables; no seed).
This migration is hand-authored because Phase 1 cannot run autogenerate yet —
the SQLAlchemy models in apps/api are placeholders. Real models will be
introduced in Phase 2 alongside repository code, and a follow-up migration
will adjust columns/types where necessary.
"""
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("character_profile_id", postgresql.UUID(as_uuid=True)),
        sa.Column("display_name", sa.String(255)),
    ...
    op.create_table(
        ...
        sa.Column("default_voice_profile_id", postgresql.UUID(as_uuid=True)),
    )
    ...
    op.create_table(
        ...
        sa.Column("audio_segment_id", postgresql.UUID(as_uuid=True)),
        sa.Column("signature", sa.String(128), nullable=False),
